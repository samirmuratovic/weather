...
function App() {
  ...
  useEffect(() => {
    async function fetchForecast() {
      const { status, current, location } = await getForecast('San Francisco')

      setForecast({ status, current, location })
    }
    fetchForecast()
  }, [])
  ...
  return (
    <article className="h-screen w-screen overflow-hidden">
      <form
        ...
        className="flex justify-center w-full pt-20 bg-blueGray-50"
      >
        <input
